  ...
  return (
    <div className="sticky top-0 flex items-start gap-2">
      <div className="flex-1 p-3 border rounded-xl bg-fd-card text-fd-card-foreground">
        <p className="text-sm font-medium mb-2">Ask Iqra AI (Coming Soon)</p>
      </div>
      <button
        aria-label="Close"
        tabIndex={-1}
...
const StorageKeyInput = '__ai_chat_input';
function ChatAIInput(props: ComponentProps<'form'>) {
  ...
  return (
    <button
      ...
      className={cn(
        ...
        'fixed bottom-4 gap-3 end-[calc(--spacing(4)+var(--removed-body-scroll-bar-size,0px))] text-fd-muted-foreground rounded-2xl shadow-lg z-20 transition-[translate,opacity]',
        open && 'translate-y-10 opacity-0',
      )}
      onClick={() => setOpen(true)}
    >
      <MessageCircleIcon className="size-4.5" />
      Ask Iqra AI (Coming Soon)
    </button>
  );
}
